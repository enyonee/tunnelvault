...
class TestParseTargets:
    @pytest.mark.parametrize("inputs,domains,networks,hosts", [
        ...
        ([], [], [], []),
        (["  *.corp.local  ", "  10.0.0.0/8  "], ["corp.local"], ["10.0.0.0/8"], []),
        (["", "  ", "10.0.0.0/8"], [], ["10.0.0.0/8"], []),
        (["999.999.999.999/99"], [], [], []),
        (["*.a.local", "*.b.local", "*.c.local"], ["a.local", "b.local", "c.local"], [], []),
        (["10.0.0.1/8"], [], ["10.0.0.1/8"], []),
        (["999.999.999.999"], [], [], []),
        (["999.0.0.1", "10.0.0.1", "*.corp.local"], ["corp.local"], [], ["10.0.0.1"]),
    ])
    ...
    def test_mixed_input(self):
        ...
        assert result.hosts == ["192.168.1.1", "git.example.com"]
...
class TestMergeTargets:
    def test_merge_into_empty(self):
        ...
        assert tcfg.dns["domains"] == ["corp.local"]
